...
export const analyzeJournalEntry = async (req, res) => {
    try {
        ...
        // Initialize Gemini model - updated to use gemini-1.5-flash or similar
        ...
        const analysisPrompt = {
            contents: [{
                parts: [{
                    text: `Analyze this workplace journal entry for ${user.name} using NLP principles. Provide a JSON response with these fields:
                    {
                        "languagePatterns": {
        ...
        const sentimentPrompt = {
            contents: [{
                parts: [{
                    text: `Rate the sentiment of this journal entry from 1-10 (1=negative, 10=positive). Return ONLY the number.
        ...
        const scoreText = sentimentResponse.text().trim();
        ...
        const journalEntry = await JournalModel.create({
            ...
            sentimentScore,
            tags,
            nlpPatterns: parseNLPPatterns(analysisText)
        });
...
const analyzeCommonThemes = async (entries) => {
    try {
        ...
        const prompt = {
            contents: [{
                parts: [{
                    text: `Identify the top 3 recurring themes in these journal entries. For each, provide:
                    - The theme name
        ...
        const themesText = response.text();

        if (!themesText) return [];
        
        try {
            const jsonString = themesText.match(/\[[\s\S]*\]/)?.[0] || '[]';
